...
PICK_A_COL = "Pick a column: [left column: enter 0, middle column: enter 1, right column enter 2]"

PICK_A_ROW = "Pick a row: [upper row: enter 0, middle row: enter 1, bottom row: enter 2]:"

def announce_win(symbol):
    print(PLAYER + symbol + " just nicked the game")
...
# This function checks if any winner is winning
    ...
    for row in range (0, 3):
        if (board[row][0] == board[row][1] == board[row][2] == symbol_1):
            winner = False
            announce_win(symbol_1)
   
        elif (board[row][0] == board[row][1] == board[row][2] == symbol_2):
            winner = False
            announce_win(symbol_2)
